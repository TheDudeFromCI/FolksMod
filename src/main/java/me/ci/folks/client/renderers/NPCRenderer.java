package me.ci.folks.client.renderers;

import com.mojang.blaze3d.matrix.MatrixStack;

import me.ci.folks.client.renderers.models.NPCModel;
...
public class NPCRenderer extends MobRenderer<NPCEntity, NPCModel> {
    ...
    public Vector3d getRenderOffset(NPCEntity pEntity, float pPartialTicks) {
        return pEntity.isCrouching() ? new Vector3d(0.0D, -0.125D, 0.0D)
                : super.getRenderOffset(pEntity, pPartialTicks);
    }
    ...
    private void setModelProperties(NPCEntity pClientPlayer) {
        ...
        if (mainArmPose.isTwoHanded()) {
            offArmPose = pClientPlayer.getOffhandItem().isEmpty() ? BipedModel.ArmPose.EMPTY
    ...
    public void renderRightHand(MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight,
            NPCEntity pPlayer) {
        this.renderHand(pMatrixStack, pBuffer, pCombinedLight, pPlayer, (this.model).rightArm,
                (this.model).rightSleeve);
    }

    public void renderLeftHand(MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight,
            NPCEntity pPlayer) {
        this.renderHand(pMatrixStack, pBuffer, pCombinedLight, pPlayer, (this.model).leftArm, (this.model).leftSleeve);
    }

    private void renderHand(MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight,
            NPCEntity pPlayer, ModelRenderer pRendererArm, ModelRenderer pRendererArmwear) {
        ...
        pRendererArm.render(pMatrixStack, pBuffer.getBuffer(RenderType.entitySolid(TEXTURE)),
                pCombinedLight, OverlayTexture.NO_OVERLAY);
        pRendererArmwear.xRot = 0.0F;
        pRendererArmwear.render(pMatrixStack,
                pBuffer.getBuffer(RenderType.entityTranslucent(TEXTURE)), pCombinedLight,
    ...
    protected void setupRotations(NPCEntity entity, MatrixStack matrix, float age, float yaw, float partialTicks) {
        ...
        if (entity.isFallFlying()) {
            ...
            if (entity.isVisuallySwimming()) {
                matrix.translate(0.0D, -1.0D, 0.3D);
            }
    ...
    private static BipedModel.ArmPose getArmPose(NPCEntity entity, Hand hand) {
        ...
        } else if (!entity.swinging && itemstack.getItem() == Items.CROSSBOW
